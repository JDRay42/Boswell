...
#[cfg(test)]
// This file *is* the crate's test module (included via `mod tests;`); the inner
// `mod tests` is stylistic. The field-reassign pattern is fine for test setup.
#[allow(clippy::module_inception, clippy::field_reassign_with_default)]
mod tests {
    use crate::{ChunkStrategy, ExtractionRequest, Extractor, ExtractorConfig};
    ...
    async fn test_extraction_metadata() {
        ...
        let extractor =
            Extractor::new(llm, store, gatekeeper, config).with_model_name("test-model");
