...
use crate::types::{
    ClaimCandidate, ClaimResult, ExtractionFailure, ExtractionMetadata, ExtractionRequest,
    ExtractionResult,
};
    ...
    S::Error: std::fmt::Display,
{
    ...
    pub fn new(llm_provider: L, store: S, gatekeeper: Gatekeeper, config: ExtractorConfig) -> Self {
        Self {
    ...
    async fn extract_single(
        ...
        // Build prompt
        let prompt_builder = PromptBuilder::new(request.text.clone(), request.namespace.clone());
    ...
    async fn extract_from_chunks(
        ...
        let chunker = TextChunker::new(self.config.chunk_strategy, self.config.max_chunk_size);
    ...
    async fn process_candidate(
        ...
        let validation = self
            .gatekeeper
            .validate(&claim, None::<&S>)
            .map_err(|e| format!("Validation error: {}", e))?;

        if validation.status != ValidationStatus::Accepted {
            return Err(format!("Validation failed: {:?}", validation.reasons));
        }
