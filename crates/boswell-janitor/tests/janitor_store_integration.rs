...
fn test_janitor_deletes_stale_ephemeral_in_sqlite() {
    ...
    store.assert_claim(claim(stale, "ephemeral", now - 20 * 3600, (0.8, 0.9))).unwrap();
    store.assert_claim(claim(fresh, "ephemeral", now - 2 * 3600, (0.8, 0.9))).unwrap();
    ...
    assert!(store.get_claim(stale).unwrap().is_none(), "stale claim should be deleted");
    assert!(store.get_claim(fresh).unwrap().is_some(), "fresh claim should remain");
}
...
fn test_janitor_demotes_decayed_task_in_sqlite() {
    ...
    let id = ClaimId::new();
    store.assert_claim(claim(id, "task", now - 20 * 3600, (0.8, 0.9))).unwrap();

    let mut janitor = Janitor::default_config();
    janitor.sweep(&mut store).unwrap();

    let after = store.get_claim(id).unwrap().expect("claim should still exist");
    assert_eq!(after.tier, "ephemeral", "decayed task claim should be demoted, not deleted");
...
fn test_janitor_leaves_fresh_high_confidence_task_alone() {
    ...
    let id = ClaimId::new();
    store.assert_claim(claim(id, "task", now - 1 * 3600, (0.85, 0.95))).unwrap();
